...
pub fn execute_tool(
    ...
    // Try to find the tool first
    let tool = get_tool_by_id(tool_name)
    ...
    match tool.category.as_str() {
        "mouse" | "keyboard" | "system" => {
            ...
            let handler = handler
                .ok_or_else(|| anyhow::anyhow!("AutomationHandler required for tool: {}", tool_name))?;
            computer_automation::execute_automation_tool(tool_name, arguments, handler)
        }
        "web" => {
            // Web search/fetch tools
            web_search::execute_web_tool(tool_name, arguments)
        }
        "delegation" => {
...
mod tests {
    ...
    fn test_tool_categories() {
        ...
        assert!(categories.contains(&"mouse".to_string()), "Missing mouse category");
        assert!(categories.contains(&"keyboard".to_string()), "Missing keyboard category");
        assert!(categories.contains(&"web".to_string()), "Missing web category");
        assert!(categories.contains(&"delegation".to_string()), "Missing delegation category");
    }
